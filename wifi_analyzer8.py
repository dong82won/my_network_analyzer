...
class NetworkDataCollector(QThread):
    ...
    def run(self):
        while self.is_running:
            ...
            self._get_wired_info(metrics)  # 유선 랜 상태 독립 감지
    ...
    def _get_wired_info(self, metrics: NetworkMetrics):
        try:
            # Linux ip link 명령으로 eth* / en* 인터페이스의 UP 상태 독립 검출
            res = subprocess.run(["ip", "-br", "link"], capture_output=True, text=True, timeout=1)
            wired_up = False
            for line in res.stdout.strip().split('\n'):
                parts = line.split()
                if len(parts) >= 2:
                    iface_name, state = parts[0], parts[1]
                    if (iface_name.startswith("en") or iface_name.startswith("eth")) and state == "UP":
                        wired_up = True
                        break
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # [핵심] QTabWidget 우측 상단 빈 공간에 유/무선 상태 표시 코너 위젯 배치
    ...
    def update_ui(self, m: NetworkMetrics):
        ...
        self.card_link.set_value(m.link_speed_mbps, link_status if m.wifi_connected else "BAD", link_color if m.wifi_connected else "#FF4444")
        ...
        self.card_freq.set_value(m.freq_ghz, freq_status if m.wifi_connected else "BAD", freq_color if m.wifi_connected else "#FF4444")
