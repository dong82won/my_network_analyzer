...
class NetworkDataCollector(QThread):
    ...
    def _get_wifi_info(self, metrics: NetworkMetrics):
        try:
            # ACTIVE, DEVICE, SSID, BSSID, SIGNAL, FREQ, RATE 순서 수집
    ...
    def _get_ip_info(self, metrics: NetworkMetrics):
        try:
            # 동적 감지된 무선 인터페이스 기반으로 IP 및 게이트웨이 조회
...
class MainWindow(QMainWindow):
    ...
    def _init_dashboard_tab(self):
        ...
        # 3. Realtime Signal History Plot (PyQtGraph: HH:MM:SS 적용 및 마우스 줌 고정)
        ...
        # [Qt 6 표준] ViewBox 직접 추출 후 인터랙션 및 우클릭 메뉴 완전 차단
        view_box = self.plot_widget.getViewBox()
        view_box.setMouseEnabled(x=False, y=False)  # 마우스 줌/이동 금지
        view_box.setMenuEnabled(False)              # 우클릭 컨텍스트 메뉴 비활성화
    ...
    def update_ui(self, m: NetworkMetrics):
        # Header Info Update (자동 인식된 interface, IP, Gateway 표시)
        ...
        # Plot Data Buffer Update (Epoch Timestamp 기반)
        now_ts = time.time()
        ...
        # [PyQtGraph/Pylance 표준] ViewBox.setRange()에 xRange 튜플 및 padding 전달
