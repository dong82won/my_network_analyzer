...
class MainWindow(QMainWindow):
    ...
    def _init_dashboard_tab(self):
        ...
        # 2. Key Metrics Grid (2행 3열 - 총 6개 개별 카드)
    ...
    def update_ui(self, m: NetworkMetrics):
        ...
        # 5. LINK SPEED (바다코끼리 연산자 := 로 Match 객체 타입 좁히기 적용)
        ...
        # 6. FREQUENCY (바다코끼리 연산자 := 로 Match 객체 타입 좁히기 적용)
        freq_val = int(match.group()) if (match := re.search(r'\d+', m.freq_ghz)) else 0
        freq_status = "GOOD" if freq_val >= 4900 else "WARN"  # 5GHz=GOOD, 2.4GHz=WARN
